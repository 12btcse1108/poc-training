...
public class DataGenWithHive {
	...
	private static String sinkTopic = "lrTopic4";
	private static String sourceTopic = "lrTopic1";
	...
	public static void main(String[] args) {
		...
		SparkConf sparkConf = new SparkConf().setAppName("DataGenWithHive")
		...
		records.map(new Function<String, List<Double>>() {
			...
			StructType schema = DataTypes
					.createStructType(Arrays.asList(DataTypes.createStructField("f0", DataTypes.DoubleType, false),
							...
							DataTypes.createStructField("f4", DataTypes.DoubleType, false),
							...
							DataTypes.createStructField("f9", DataTypes.DoubleType, false),
							DataTypes.createStructField("f10", DataTypes.DoubleType, false)));
			sqlContext.read().schema(schema).json(jsonMsg).write().mode(SaveMode.Append)
					.saveAsTable("default.table_lr");
